...
alertmanager:
  ...
  config:
    ...
    - name: 'slack-notifications'
      slack_configs:
      - channel: '#fourtimes'
        title: '[{{ .Status | toUpper }}{{ if eq .Status "firing" }}:{{ .Alerts.Firing | len }}{{ end }}] Tiscot Prometheus Alerts'
        text: >-
          {{ range .Alerts }}
...
**Open the browser to load the Domain Name**

```bash
http://grafana.xxxx.co
    => username: admin
